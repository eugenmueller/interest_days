...
RSpec.describe InterestDays::Calculator do
  ...
  describe "isda act act strategy based calculation" do
    let(:strategy) { :isda_act_act }

    it { expect(calculator.strategy).to be(InterestDays::Calculation::IsdaActAct) }
  end
  ...
  describe "when strategy not available" do
    ...
    it { expect { calculator.interest_day_count_factor }.to raise_error(StandardError) }
  end
end
